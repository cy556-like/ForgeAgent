...
def create_chat(username: str, title: str = "新对话", mode: str = "agent", agent_id: str = None) -> dict:
    ...
    Args:
        ...
        agent_id: 智能体ID，用于关联对话与智能体（跨浏览器同步）

    Returns:
        dict: 包含 chat_id、title、mode 和 agent_id
    """
...
def list_chats(username: str, mode: str = None, agent_id: str = None) -> list[dict]:
    """列出用户的会话，按更新时间倒序

    Args:
        mode: 可选，按模式过滤 "agent" 或 "chat"，为 None 则返回全部
        agent_id: 可选，按智能体ID过滤，为 None 则返回全部
    """
    chats = _load_user_chats(username)
    ...
    # 按 agent_id 过滤（如果指定了 agent_id）
    if agent_id is not None:
        # agent_id 为空字符串时匹配没有关联智能体的会话
        chats = [c for c in chats if c.get("agent_id", "") == agent_id]
    return chats
