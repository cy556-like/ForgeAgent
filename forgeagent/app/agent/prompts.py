...
| 文档修改 | 修改知识库中已有文档的内容（支持追加或替换），可返回docx文件 | modify_document_tool |
| 文档导出 | 生成新的docx文档供下载（整合/汇总/简略版） | export_document_tool |
...
- **返回docx文件**（用户说"给我文件""下载文档""返回docx"）：modify_document_tool(filename="xxx.docx", content="...", return_docx=True)
- **修改流程**：
...
- **生成新文档**（不影响知识库）：export_document_tool(content="文档内容", filename="输出文件名.docx", title="文档标题")
- **典型场景**：
  - 用户要求"整合知识库文档"：先用 search_documents_tool 搜索相关内容，再整理后用 export_document_tool 生成
  - 用户要求"生成简略版"：搜索完整内容后提取要点，用 export_document_tool 导出
  - 用户要求"给我一个docx文件"：用 export_document_tool 生成下载链接
- ⚠️ 此工具不会修改知识库中的文档，仅生成新的docx文件供下载
- ⚠️ 返回的下载链接告诉用户即可，用户可在浏览器中打开链接下载文件
